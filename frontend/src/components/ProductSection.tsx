    ...
    <section className="max-w-7xl mx-auto py-12 px-4">
      ...
      <div className="flex overflow-x-auto gap-6 pb-8 hide-scrollbar snap-x">
        {products.map((p, i) => (
          ...
          >
            <button 
              onClick={() => addToCart({ id: p.id, name: p.name, price: p.price, category: p.category, image: p.image, unit: p.unit || '1 Unidad' })}
              className="absolute top-5 right-5 w-10 h-10 bg-white border border-gray-100 rounded-full shadow-lg flex items-center justify-center text-ananas-green hover:bg-ananas-green hover:text-white transition-all z-10 hover:scale-110"
            >
